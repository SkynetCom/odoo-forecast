# Copyright 2019-21 ForgeFlow, S.L.
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).
# Archivo 1: Modelos Python

from odoo import fields, models

class StockReportByLocation(models.TransientModel):
    _name = "stock.report.quantity.by.location"
    _description = "Stock Report Quantity By Location"

    product_id = fields.Many2one("product.product", string="Product", required=True)
    product_category_id = fields.Many2one("product.category", string="Product Category")
    uom_id = fields.Many2one("uom.uom", string="Unit of Measure")
    quantity_on_hand = fields.Float(string="Quantity On Hand", default=0.0)
    quantity_reserved = fields.Float(string="Quantity Reserved", default=0.0)
    quantity_unreserved = fields.Float(string="Quantity Unreserved", default=0.0)
    forecast_quantity = fields.Float(string="Forecast Quantity", default=0.0)
    incoming_quantity = fields.Float(string="Incoming Quantity", default=0.0)
    outgoing_quantity = fields.Float(string="Outgoing Quantity", default=0.0)
    location_id = fields.Many2one("stock.location", string="Location", required=True)
    wiz_id = fields.Many2one("stock.report.quantity.by.location.prepare", string="Wizard")
    default_code = fields.Char(string="Default Code")


class StockReportByLocationPrepare(models.TransientModel):
    _name = "stock.report.quantity.by.location.prepare"
    _description = "Stock Report Quantity By Location Prepare"

    location_ids = fields.Many2many(
        comodel_name="stock.location", string="Locations", required=True
    )
    with_quantity = fields.Boolean(
        string="Forecast Quantity > 0",
        default=True,
        help="Show only the products that have existing forecast quantity greater than 0",
    )

    def open(self):
        self.ensure_one()
        self._compute_stock_report_by_location()
        action = {
            "type": "ir.actions.act_window",
            "view_mode": "pivot,tree",
            "name": "Stock Report by Location",
            "context": {
                "search_default_forecast_gt_zero": 1 if self.with_quantity else 0,
                "group_by_no_leaf": 1,
                "group_by": [],
            },
            "res_model": "stock.report.quantity.by.location",
            "domain": [("wiz_id", "=", self.id)],
        }
        return action

    def _compute_stock_report_by_location(self):
        self.ensure_one()
        vals_list = []
        for loc in self.location_ids:
            quant_groups = self.env["stock.quant"].read_group(
                [("location_id", "child_of", [loc.id])],
                ["quantity", "reserved_quantity", "product_id"],
                ["product_id"],
            )
            mapping = {}
            for quant_group in quant_groups:
                qty_on_hand = quant_group["quantity"]
                qty_reserved = quant_group["reserved_quantity"]
                qty_unreserved = qty_on_hand - qty_reserved
                qty_dict = {
                    "quantity_on_hand": qty_on_hand,
                    "quantity_reserved": qty_reserved,
                    "quantity_unreserved": qty_unreserved,
                }
                mapping.setdefault(quant_group["product_id"][0], qty_dict)

            moves_in = self.env["stock.move"].read_group(
                [
                    ("location_dest_id", "child_of", [loc.id]),
                    ("state", "in", ["waiting", "confirmed", "assigned", "partially_available"]),
                ],
                ["product_id", "product_qty"],
                ["product_id"],
            )

            moves_out = self.env["stock.move"].read_group(
                [
                    ("location_id", "child_of", [loc.id]),
                    ("state", "in", ["waiting", "confirmed", "assigned", "partially_available"]),
                ],
                ["product_id", "product_qty"],
                ["product_id"],
            )

            incoming_mapping = {
                move_in["product_id"][0]: move_in["product_qty"] for move_in in moves_in
            }
            outgoing_mapping = {
                move_out["product_id"][0]: move_out["product_qty"] for move_out in moves_out
            }

            products = self.env["product.product"].search([("type", "=", "product")])
            for product in products:
                qty_dict = mapping.get(product.id, {})
                qty_on_hand = qty_dict.get("quantity_on_hand", 0.0)
                qty_reserved = qty_dict.get("quantity_reserved", 0.0)
                qty_unreserved = qty_dict.get("quantity_unreserved", 0.0)

                incoming_qty = incoming_mapping.get(product.id, 0.0)
                outgoing_qty = outgoing_mapping.get(product.id, 0.0)

                forecast_qty = qty_on_hand + incoming_qty - outgoing_qty

                if not self.with_quantity or forecast_qty > 0:
                    vals_list.append(
                        {
                            "product_id": product.id,
                            "product_category_id": product.categ_id.id,
                            "uom_id": product.uom_id.id,
                            "quantity_on_hand": qty_on_hand,
                            "quantity_reserved": qty_reserved,
                            "quantity_unreserved": qty_unreserved,
                            "forecast_quantity": forecast_qty,
                            "incoming_quantity": incoming_qty,
                            "outgoing_quantity": outgoing_qty,
                            "location_id": loc.id,
                            "wiz_id": self.id,
                            "default_code": product.default_code,
                        }
                    )
        self.env["stock.report.quantity.by.location"].create(vals_list)
